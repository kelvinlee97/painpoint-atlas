...
def load_dashboard_payload(path: str) -> dict:
    ...
    try:
        ...
        latest_run = database.connection.execute(
            "SELECT created_at, details FROM runs WHERE kind = 'run' AND status = 'success' ORDER BY id DESC LIMIT 1"
        ).fetchone()
        run_details = json.loads(latest_run[1]) if latest_run else {}
        return {
            "analysis": {
                "generated_at": latest_run[0] if latest_run else None,
                "clustered_evidence": run_details.get("clustered_evidence"),
            },
            "summary": {
...
* {{ box-sizing:border-box; }} html {{ scroll-behavior:smooth; }} body {{ margin:0; background:var(--surface); color:var(--text); font:17px/1.75 "Fira Sans",ui-sans-serif,system-ui,sans-serif; overflow-x:hidden; }} body,button,input,select {{ -webkit-font-smoothing:antialiased; }} main {{ max-width:1240px; margin:auto; padding:28px 24px 72px; }} h1,h2,h3,p {{ margin-top:0; }} h1 {{ color:var(--ink); font-size:clamp(28px,4vw,42px); letter-spacing:-.035em; line-height:1.1; margin-bottom:10px; }} h2 {{ color:var(--ink); font-size:20px; letter-spacing:-.015em; margin-bottom:4px; }} h3 {{ color:var(--ink); font-size:20px; line-height:1.25; margin-bottom:6px; }} a {{ color:var(--primary); font-weight:650; }} .muted,.caption,.helper {{ color:var(--muted); }}
...
.summary {{ display:flex; flex-wrap:wrap; gap:12px 28px; margin:24px 0 4px; font-size:14px; color:var(--muted); }} .summary strong {{ color:var(--ink); }} .scope-note {{ margin-bottom:30px; max-width:85ch; }}
.feature {{ background:var(--soft-blue); border-left:4px solid var(--primary); border-radius:12px; padding:28px; }} .feature h2 {{ font-size:clamp(23px,3vw,30px); }} .feature p {{ max-width:65ch; }} .feature blockquote {{ max-width:65ch; border-left:2px solid #93c5fd; padding:4px 0 4px 20px; margin:22px 0; }} .feature footer {{ font-size:13px; color:var(--muted); margin-top:8px; }}
.rank-name a {{ line-height:1.6; text-underline-offset:4px; }} .card {{ scroll-margin-top:20px; border-left:0; }} .card-summary {{ max-width:75ch; }} .reading-flow {{ max-width:72ch; margin:24px 0; }} .reading-flow > div {{ margin-bottom:18px; }} .reading-flow dt {{ font-weight:700; color:var(--ink); margin-bottom:5px; }} .reading-flow dd {{ margin:0; }} .next-step {{ background:var(--soft-blue); padding:20px; border-radius:8px; margin:24px 0; max-width:76ch; }} .next-step h4 {{ margin:0 0 8px; }} .next-step p {{ margin-bottom:8px; }} .featured-quote {{ margin-top:22px; }} .featured-quote .quote {{ background:var(--surface); }} .disclosure summary::after {{ content:'＋'; }} .disclosure[open] summary::after {{ content:'−'; }} .disclosure summary > span[aria-hidden] {{ display:none; }}
@media(max-width:560px) {{ .feature {{ padding:20px; }} .masthead h1 {{ font-size:30px; line-height:1.3; }} .rank-name a {{ overflow-wrap:anywhere; }} .quote-meta {{ flex-wrap:wrap; }} }}
@media (prefers-reduced-motion:reduce) {{ *,*::before,*::after {{ scroll-behavior:auto !important; animation-duration:.01ms !important; animation-iteration-count:1 !important; transition-duration:.01ms !important; }} }}
</style></head>
<body><a class="skip-link" href="#opportunity-list">跳到机会列表</a><main>
<header class="masthead"><div><div class="eyebrow">Painpoint Atlas / Opportunity Radar</div><h1>从用户的抱怨里，<br>找到值得解决的问题。</h1><p class="lead">阅读 App Store 与 Google Play 的真实差评，了解谁被什么问题困住，再决定下一步验证什么。</p></div><div class="meta"><strong>DATA SNAPSHOT</strong><span id="analysis-date"></span></div></header>
<section id="featured" aria-label="本期发现"></section>
<div class="summary" id="summary" aria-label="数据摘要"></div>
<p class="helper scope-note" id="analysis-scope"></p>
<section class="panel" aria-labelledby="ranking-title"><div class="section-heading"><div><h2 id="ranking-title">本期问题索引</h2><p class="caption">点击标题阅读详情。分数仅用于安排研究顺序，不代表成功概率。</p></div><span class="caption" id="ranking-scope">全部</span></div><ol class="ranking" id="ranking" aria-label="机会优先级排行榜"></ol></section>
<section class="panel" aria-labelledby="filters-title"><h2 id="filters-title">找到你关心的问题</h2><p class="caption">按问题、产品或判断筛选，先读用户经历，再看完整分析。</p><div class="toolbar"><div class="field field-search"><label for="search">查找证据</label><input id="search" type="search" placeholder="搜索问题、App、根因或商业判断" aria-describedby="search-help" autocomplete="off"><p class="helper" id="search-help">搜索会同时匹配机会标题、分析字段和来源 App。</p></div><div class="field"><label for="decision">商业判断</label><select id="decision"><option value="">全部判断</option><option>值得优先验证</option><option>优先作为避坑规则</option><option>暂不进入</option></select></div><button class="reset-button" id="reset" type="button">清除筛选</button></div><p class="result-count" id="result-count" aria-live="polite"></p></section>
...
function render() {{
  ...
  document.querySelector('#analysis-date').textContent = analysis.generated_at ? `分析于 ${{analysis.generated_at}} UTC` : '分析日期未记录';
  document.querySelector('#summary').innerHTML = [['累计产品',s.apps],['累计评论',s.reviews],['累计提取证据',s.evidence],['本期问题',s.opportunities]].map(x => `<span>${{x[0]}} <strong>${{count(x[1])}}</strong></span>`).join('');
  document.querySelector('#analysis-scope').textContent = `本次聚类：${{analysis.clustered_evidence == null ? '样本数量未记录' : count(analysis.clustered_evidence) + ' 条证据'}}。累计数据不等于本次分析样本；评论与模型判断不能替代需求验证。`;
  const featured = DATA.opportunities[0];
  document.querySelector('#featured').innerHTML = featured ? `<article class="feature"><p class="eyebrow">本期先读 · 按研究优先级推荐</p><h2>${{esc(featured.label)}}</h2><p>${{esc(featured.summary || '暂无摘要')}}</p>${{featured.evidence?.length ? `<blockquote>${{esc(featured.evidence[0].quote)}}<footer>${{esc(featured.evidence[0].app_name)}} · 用户原文</footer></blockquote>` : ''}}<a href="#opportunity-list">阅读发现与证据 ↓</a></article>` : '';
  ...
  document.querySelector('#featured').hidden = hasFilters;
  ...
  document.querySelector('#opportunity-list').innerHTML = items.length ? items.map((item, index) => {{
    ...
    const evidenceArticles = evidence.map(row => {{
    ...
    const evidenceMarkup = evidenceArticles.join('') || '<p class="muted">暂无原始证据。</p>';
    return `<article class="card" id="opportunity-${{index}}" tabindex="-1"><div class="card-head"><div><div class="card-kicker">机会 ${{String(index + 1).padStart(2,'0')}}</div><h3>${{esc(item.label || '未命名机会')}}</h3><p class="card-summary">${{esc(item.summary || '暂无摘要')}}</p></div><span class="decision ${{decisionClass(decision)}}">${{esc(decision)}}</span></div><ul class="facts" aria-label="机会指标"><li class="fact"><strong>${{number(item.score).toFixed(1)}}</strong> 优先级</li><li class="fact"><strong>${{count(item.review_count)}}</strong> 条评论 / <strong>${{count(item.app_count)}}</strong> 个 App</li><li class="fact">严重度 <strong>${{number(item.average_severity).toFixed(1)}}/5</strong></li><li class="fact">付费信号 <strong>${{number(item.average_paid_signal).toFixed(1)}}/3</strong></li></ul>${{evidence.length ? `<div class="featured-quote"><p class="helper">听听用户怎么说 · 原文</p>${{evidenceArticles[0]}}</div>` : ''}}<dl class="reading-flow"><div><dt>谁被困住了</dt><dd>${{esc(item.affected_user || '待补充')}}</dd></div><div><dt>这对用户意味着什么</dt><dd>${{esc(item.user_consequence || '待补充')}}</dd></div></dl><aside class="next-step"><h4>你可以先做的一步</h4><p>找 5 位遇过这类问题的用户，请他们回顾最近一次失败、当时的替代办法，以及为解决问题投入的时间或费用。先确认问题是否反复发生，再决定要不要做原型。</p><span class="helper">这是访谈起点，不是已验证的需求。</span></aside><details class="disclosure"><summary>完整分析与研究建议 <span aria-hidden="true">＋</span></summary><div class="details-body"><dl class="reading-flow"><div><dt>失败阶段</dt><dd>${{esc(item.failure_stage || '待补充')}}</dd></div><div><dt>可能的原因 · 模型推断</dt><dd>${{esc(item.root_cause || '待补充')}}</dd></div><div><dt>商业判断 · 待验证</dt><dd>${{esc(item.commercial_implication || '待补充')}}</dd></div><div><dt>原始验证建议</dt><dd>${{esc(item.validation_action || '待补充')}}</dd></div><div><dt>分析置信度 ${{number(item.analysis_confidence).toFixed(2)}}</dt><dd>模型输出的自评数值，未经校准，不代表结论正确的概率。涉及内部日志或埋点的建议，仅适用于能访问相应产品数据的团队。</dd></div></dl></div></details><details class="disclosure"><summary>涉及产品 <span class="summary-count">${{apps.length}} 个</span></summary><div class="details-body">${{appMarkup}}</div></details><details class="disclosure"><summary>全部原始证据 <span class="summary-count">${{evidence.length}} 条</span></summary><div class="details-body">${{evidenceMarkup}}</div></details></article>`;
  }}).join('') : `<div class="empty">${{hasFilters ? '没有匹配的问题，试试其他关键词或清除筛选。' : '本期暂无达到证据门槛的问题，请稍后再来。'}}</div>`;
}}
