...
Painpoint Atlas 帮助独立开发者和产品团队阅读 App Store 与 Google Play 的真实差评：谁被什么问题困住，付出了什么代价，下一步可以怎样验证。

[阅读 Dashboard](https://kelvinlee97.github.io/painpoint-atlas/) · [查看一份示例报告](reports/2026-09-03.md#executive-summary)

![Painpoint Atlas：本期发现与用户证据](docs/dashboard.png)

### 你会读到什么？

例如，日程工具的用户反映：编辑或删除事件时反复卡住，问题已经持续数月。报告将这条原文与其他产品的相关反馈放在一起，帮助你判断：这是值得深入访谈的问题吗？用户现在如何绕过它？

每个问题都附有用户原文、涉及产品和待验证的分析。**有抱怨不等于有市场，排序也不是成功概率。** 示例报告使用 20 条证据进行聚类；累计采集数据与本次样本分别展示。

### 先在本机读一读

需要 Python 3.11+，无需安装额外依赖。仓库已有数据时，直接启动：

```bash
python3 -m opportunity_radar dashboard
```

打开 <http://127.0.0.1:8000>。想采集新评论并生成分析，再按照下方「本地运行」配置 API key。
...
默认每天 **02:30 UTC** 运行，也可以手动触发。每次推送到 `main` 都会重新采集和分析、持久化结果并部署；自动持久化合并带有 `[skip ci]`，不会形成刷新循环。若新分析未产生达到门槛的机会，系统会保存新证据但保留上一期分析，避免偶发模型退化清空 Dashboard。
